...
interface ReportModalProps {
  isOpen: boolean;
  onClose: () => void;
  // Mode A: PSQ Analytics (Your Teammate's)
  stats?: PsqAnalytics | null;
  // Mode B: Flagging Content (Your Code)
  entry?: { question: string; answer: string; } | null;
  onSubmit?: (reason: string) => Promise<void>;
}

// --- PDF Styles (Teammate's Code) ---
...
export default function ReportModal({ isOpen, onClose, stats, entry, onSubmit }: ReportModalProps) {
  ...
  // --- DETERMINE TITLE & CONTENT BASED ON PROPS ---
  const isPsqMode = !!stats; // If stats exist, we are in PDF mode
        ...
        <div className="fixed inset-0 flex items-center justify-center p-4">
          <Dialog.Panel className="w-full max-w-md transform overflow-hidden rounded-2xl bg-white p-6 text-left align-middle shadow-xl transition-all">
            ...
            <Dialog.Title as="h3" className="text-lg font-bold leading-6 text-gray-900">
              {isPsqMode ? "Export Analysis" : "Report an Issue"}
            </Dialog.Title>
              ...
              <>
                <div className="mt-2">
                  <p className="text-sm text-gray-500">
                    Download a PDF summary of your current PSQ readiness.
                  </p>
                </div>
            ...
            {/* Close button for PSQ mode only (Form has its own buttons) */}
            {isPsqMode && (
